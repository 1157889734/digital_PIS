...
struct udp_pcb *pUdpTestRcvPcb;
struct udp_pcb *pUdpTestSendPcb;
...
void LwipInit(void)
{
	...
	IP4_ADDR(&AllBroadIpAddr, NetLocalIp[0], NetLocalIp[1], NetLocalIp[2], 255);
...
//	err_t err;
	...
	pUdpTestRcvPcb = udp_new();
  
	/* Bind the upcb to the UDP_PORT port */
	/* Using IP_ADDR_ANY allow the upcb to be used by any local interface */
	udp_bind(pUdpTestRcvPcb, IP_ADDR_ANY, PORT_TEST_RCV_L);
      		
	/* Set a receive callback for the upcb */
	udp_recv(pUdpTestRcvPcb, UdpTestReceiveCallback, NULL);
...
void UdpTestReceiveCallback(void *arg, struct udp_pcb *upcb, struct pbuf *p, struct ip_addr *addr, u16_t port)
{
	...
	UdpTestReceiveProcess((u8*)(p->payload), (int)(p->len));
...
{
	/* Create a new UDP control block  */
	pUdpTestSendPcb = udp_new();
  
	/* Bind the upcb to the UDP_PORT port */
	/* Using IP_ADDR_ANY allow the upcb to be used by any local interface */
	udp_bind(pUdpTestSendPcb, IP_ADDR_ANY, PORT_TEST_SND_L);
}
...
void UdpTestSend(u8* pbuf, int len)
{
	...
	udp_connect(pUdpTestSendPcb, &AllBroadIpAddr, PORT_TEST_RCV_R);
	...
	if (p != NULL)
	{
		...
		/* send udp data */
		udp_send(pUdpTestSendPcb, p); 

		/* free the UDP connection, so we can accept new clients */
		udp_disconnect(pUdpTestSendPcb);

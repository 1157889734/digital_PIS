...
struct ip_addr LocalBroadIpAddr;
...
struct udp_pcb *pUdpDownloadRcvPcb;
struct udp_pcb *pUdpDownloadSendPcb;
...
void UdpDownloadReceiveCallback(void *arg, struct udp_pcb *upcb, struct pbuf *p, struct ip_addr *addr, u16_t port);
...
void LwipInit(void)
{
	...
	IP4_ADDR(&LocalBroadIpAddr, NetLocalIp[0], NetLocalIp[1], NetLocalIp[2], 255);
...
void UdpTestSend(void)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpTestPcb, &LocalBroadIpAddr, PORT_TEST_R);
...
void UdpCommanderSend(u8* pbuf, int len)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpCommanderPcb, &LocalBroadIpAddr, PORT_COMMANDER_R);
...
void UdpControlSend(u8* pbuf, int len)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpControlSendPcb, &LocalBroadIpAddr, PORT_CONTROL_SND_R);
...
void UdpBroadcastSend(u8* pbuf, int len)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpBroadcastSendPcb, &LocalBroadIpAddr, PORT_BROADCAST_SND_R);
...
void UdpTalkSend(u8* pbuf, int len)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpTalkSendPcb, &LocalBroadIpAddr, PORT_TALK_SND_R);
...
void UdpPlayclkSend(u8* pbuf, int len)
{
	...
	/* Connect to the remote client */
	udp_connect(pUdpPlayclkSendPcb, &LocalBroadIpAddr, PORT_PLAYCLK_SND_R);
...
void UdpDownloadRcvInit(void)
{
   
	/* Create a new UDP control block  */
	pUdpDownloadRcvPcb = udp_new();
  
	/* Bind the upcb to the UDP_PORT port */
	/* Using IP_ADDR_ANY allow the upcb to be used by any local interface */
	udp_bind(pUdpDownloadRcvPcb, IP_ADDR_ANY, PORT_DOWNLOAD_RCV_L);
      		
	/* Set a receive callback for the upcb */
	udp_recv(pUdpDownloadRcvPcb, UdpDownloadReceiveCallback, NULL);
...
void UdpDownloadReceiveCallback(void *arg, struct udp_pcb *upcb, struct pbuf *p, struct ip_addr *addr, u16_t port)
{
	...
	NetDownloadReceiveProcess((u8*)(p->payload), (int)(p->len));
...
void UdpDownloadSendInit(void)
{
   
	/* Create a new UDP control block  */
	pUdpDownloadSendPcb = udp_new();
  
	/* Bind the upcb to the UDP_PORT port */
	/* Using IP_ADDR_ANY allow the upcb to be used by any local interface */
	udp_bind(pUdpDownloadSendPcb, IP_ADDR_ANY, PORT_DOWNLOAD_SND_L);
      		
   
}


void UdpDownloadSend(u8* pbuf, int len)
{

	struct pbuf *p;

	/* Connect to the remote client */
	udp_connect(pUdpDownloadSendPcb, &LocalBroadIpAddr, PORT_DOWNLOAD_SND_R);
	...
	if (p != NULL)
	{
		...
		/* send udp data */
		udp_send(pUdpDownloadSendPcb, p); 

		/* free the UDP connection, so we can accept new clients */
		udp_disconnect(pUdpDownloadSendPcb);

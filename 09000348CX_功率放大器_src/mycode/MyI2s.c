...
void L3Uda1341Down(void)
{
	L3SendByte((0x14 | 0x02), L3MODE_ADDR);		//selecte status

	L3SendByte(0x80, L3MODE_DATA);

}


void L3Uda1341Up(void)
{
	L3SendByte((0x14 | 0x02), L3MODE_ADDR);		//selecte status

	L3SendByte(0x83, L3MODE_DATA);
...
int L3Uda1341SetVolume(u8 dbvalue)
{
	...
	else if(dbvalue < 61)
	{
		...
		if(dbvalue==40)
		{
			L3SendByte((0x14 | 0x00), L3MODE_ADDR);
			L3SendByte(0x85, L3MODE_DATA);
			L3SendByte(dbvalue, L3MODE_DATA);
